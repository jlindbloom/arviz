(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("860c7820-5303-42f9-b96d-616c37e77c1f");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '860c7820-5303-42f9-b96d-616c37e77c1f' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"e00b090e-2120-43ba-8551-f2822aea9dba":{"defs":[],"roots":{"references":[{"attributes":{},"id":"17869","type":"AllLabels"},{"attributes":{},"id":"17812","type":"DataRange1d"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17835","type":"PolyAnnotation"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"17876"},"selection_policy":{"id":"17875"}},"id":"17852","type":"ColumnDataSource"},{"attributes":{},"id":"17829","type":"WheelZoomTool"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17860","type":"Line"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17864","type":"Title"},{"attributes":{},"id":"17810","type":"DataRange1d"},{"attributes":{"callback":null},"id":"17833","type":"HoverTool"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17855","type":"Line"},{"attributes":{},"id":"17874","type":"Selection"},{"attributes":{"overlay":{"id":"17835"}},"id":"17830","type":"LassoSelectTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17852"},"glyph":{"id":"17853"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17855"},"nonselection_glyph":{"id":"17854"},"view":{"id":"17857"}},"id":"17856","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17834","type":"BoxAnnotation"},{"attributes":{},"id":"17873","type":"UnionRenderers"},{"attributes":{},"id":"17875","type":"UnionRenderers"},{"attributes":{},"id":"17819","type":"BasicTicker"},{"attributes":{},"id":"17827","type":"PanTool"},{"attributes":{"source":{"id":"17845"}},"id":"17850","type":"CDSView"},{"attributes":{"coordinates":null,"data_source":{"id":"17858"},"glyph":{"id":"17859"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17861"},"nonselection_glyph":{"id":"17860"},"view":{"id":"17863"}},"id":"17862","type":"GlyphRenderer"},{"attributes":{"below":[{"id":"17818"}],"center":[{"id":"17821"},{"id":"17825"},{"id":"17851"}],"height":500,"left":[{"id":"17822"}],"output_backend":"webgl","renderers":[{"id":"17849"},{"id":"17856"},{"id":"17862"}],"title":{"id":"17864"},"toolbar":{"id":"17836"},"toolbar_location":null,"width":500,"x_range":{"id":"17810"},"x_scale":{"id":"17814"},"y_range":{"id":"17812"},"y_scale":{"id":"17816"}},"id":"17809","subtype":"Figure","type":"Plot"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"17851","type":"BoxAnnotation"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17853","type":"Line"},{"attributes":{},"id":"17871","type":"BasicTickFormatter"},{"attributes":{},"id":"17872","type":"AllLabels"},{"attributes":{"axis":{"id":"17822"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17825","type":"Grid"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17847","type":"Line"},{"attributes":{},"id":"17832","type":"SaveTool"},{"attributes":{},"id":"17823","type":"BasicTicker"},{"attributes":{},"id":"17814","type":"LinearScale"},{"attributes":{},"id":"17877","type":"UnionRenderers"},{"attributes":{},"id":"17876","type":"Selection"},{"attributes":{"children":[[{"id":"17809"},0,0]]},"id":"17882","type":"GridBox"},{"attributes":{},"id":"17868","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"17834"}},"id":"17828","type":"BoxZoomTool"},{"attributes":{"toolbars":[{"id":"17836"}],"tools":[{"id":"17826"},{"id":"17827"},{"id":"17828"},{"id":"17829"},{"id":"17830"},{"id":"17831"},{"id":"17832"},{"id":"17833"}]},"id":"17883","type":"ProxyToolbar"},{"attributes":{"coordinates":null,"data_source":{"id":"17845"},"glyph":{"id":"17846"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17848"},"nonselection_glyph":{"id":"17847"},"view":{"id":"17850"}},"id":"17849","type":"GlyphRenderer"},{"attributes":{},"id":"17831","type":"UndoTool"},{"attributes":{"source":{"id":"17858"}},"id":"17863","type":"CDSView"},{"attributes":{"source":{"id":"17852"}},"id":"17857","type":"CDSView"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17848","type":"Line"},{"attributes":{"toolbar":{"id":"17883"},"toolbar_location":"above"},"id":"17884","type":"ToolbarBox"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17846","type":"Line"},{"attributes":{"axis":{"id":"17818"},"coordinates":null,"group":null,"ticker":null},"id":"17821","type":"Grid"},{"attributes":{"children":[{"id":"17884"},{"id":"17882"}]},"id":"17885","type":"Column"},{"attributes":{"tools":[{"id":"17826"},{"id":"17827"},{"id":"17828"},{"id":"17829"},{"id":"17830"},{"id":"17831"},{"id":"17832"},{"id":"17833"}]},"id":"17836","type":"Toolbar"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17874"},"selection_policy":{"id":"17873"}},"id":"17845","type":"ColumnDataSource"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17861","type":"Line"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17859","type":"Line"},{"attributes":{},"id":"17816","type":"LinearScale"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"17854","type":"Line"},{"attributes":{},"id":"17878","type":"Selection"},{"attributes":{"data":{},"selected":{"id":"17878"},"selection_policy":{"id":"17877"}},"id":"17858","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"formatter":{"id":"17871"},"group":null,"major_label_policy":{"id":"17872"},"ticker":{"id":"17819"}},"id":"17818","type":"LinearAxis"},{"attributes":{},"id":"17826","type":"ResetTool"},{"attributes":{"coordinates":null,"formatter":{"id":"17868"},"group":null,"major_label_policy":{"id":"17869"},"ticker":{"id":"17823"}},"id":"17822","type":"LinearAxis"}],"root_ids":["17885"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"e00b090e-2120-43ba-8551-f2822aea9dba","root_ids":["17885"],"roots":{"17885":"860c7820-5303-42f9-b96d-616c37e77c1f"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();