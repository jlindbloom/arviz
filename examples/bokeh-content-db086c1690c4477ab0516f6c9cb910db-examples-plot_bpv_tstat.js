(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("ed5a6ca3-5838-43d6-a772-6c0497b2346d");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'ed5a6ca3-5838-43d6-a772-6c0497b2346d' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"5b410095-192d-449d-bed1-2338e7913fbc":{"defs":[],"roots":{"references":[{"attributes":{"tools":[{"id":"17963"},{"id":"17964"},{"id":"17965"},{"id":"17966"},{"id":"17967"},{"id":"17968"},{"id":"17969"},{"id":"17970"}]},"id":"17973","type":"Toolbar"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17972","type":"PolyAnnotation"},{"attributes":{},"id":"17951","type":"LinearScale"},{"attributes":{"coordinates":null,"data_source":{"id":"17982"},"glyph":{"id":"17983"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17985"},"nonselection_glyph":{"id":"17984"},"view":{"id":"17987"}},"id":"17986","type":"GlyphRenderer"},{"attributes":{},"id":"18029","type":"UnionRenderers"},{"attributes":{"coordinates":null,"data_source":{"id":"17988"},"glyph":{"id":"17989"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17991"},"nonselection_glyph":{"id":"17990"},"view":{"id":"17993"}},"id":"17992","type":"GlyphRenderer"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"18009"}]},"id":"18008","type":"Legend"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17983","type":"Line"},{"attributes":{"toolbar":{"id":"18035"},"toolbar_location":"above"},"id":"18036","type":"ToolbarBox"},{"attributes":{"axis":{"id":"17955"},"coordinates":null,"group":null,"ticker":null},"id":"17958","type":"Grid"},{"attributes":{"data":{},"selected":{"id":"18005"},"selection_policy":{"id":"18004"}},"id":"17988","type":"ColumnDataSource"},{"attributes":{"data":{},"selected":{"id":"18030"},"selection_policy":{"id":"18029"}},"id":"18010","type":"ColumnDataSource"},{"attributes":{},"id":"17964","type":"PanTool"},{"attributes":{},"id":"17956","type":"BasicTicker"},{"attributes":{"children":[[{"id":"17946"},0,0]]},"id":"18034","type":"GridBox"},{"attributes":{},"id":"17963","type":"ResetTool"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18016","type":"Title"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18011","type":"Circle"},{"attributes":{"source":{"id":"17982"}},"id":"17987","type":"CDSView"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17984","type":"Line"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18012","type":"Circle"},{"attributes":{},"id":"18030","type":"Selection"},{"attributes":{"overlay":{"id":"17972"}},"id":"17967","type":"LassoSelectTool"},{"attributes":{},"id":"17960","type":"BasicTicker"},{"attributes":{},"id":"18003","type":"Selection"},{"attributes":{"coordinates":null,"data_source":{"id":"18010"},"glyph":{"id":"18011"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18013"},"nonselection_glyph":{"id":"18012"},"view":{"id":"18015"}},"id":"18014","type":"GlyphRenderer"},{"attributes":{},"id":"18005","type":"Selection"},{"attributes":{},"id":"18004","type":"UnionRenderers"},{"attributes":{"axis":{"id":"17959"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17962","type":"Grid"},{"attributes":{},"id":"17969","type":"SaveTool"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17985","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17991","type":"Line"},{"attributes":{},"id":"17998","type":"AllLabels"},{"attributes":{},"id":"18002","type":"UnionRenderers"},{"attributes":{"toolbars":[{"id":"17973"}],"tools":[{"id":"17963"},{"id":"17964"},{"id":"17965"},{"id":"17966"},{"id":"17967"},{"id":"17968"},{"id":"17969"},{"id":"17970"}]},"id":"18035","type":"ProxyToolbar"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18003"},"selection_policy":{"id":"18002"}},"id":"17982","type":"ColumnDataSource"},{"attributes":{"source":{"id":"18010"}},"id":"18015","type":"CDSView"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17971","type":"BoxAnnotation"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17990","type":"Line"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17989","type":"Line"},{"attributes":{},"id":"18001","type":"AllLabels"},{"attributes":{"callback":null},"id":"17970","type":"HoverTool"},{"attributes":{"source":{"id":"17988"}},"id":"17993","type":"CDSView"},{"attributes":{"overlay":{"id":"17971"}},"id":"17965","type":"BoxZoomTool"},{"attributes":{},"id":"17997","type":"BasicTickFormatter"},{"attributes":{},"id":"17947","type":"DataRange1d"},{"attributes":{"coordinates":null,"formatter":{"id":"18000"},"group":null,"major_label_policy":{"id":"18001"},"ticker":{"id":"17956"}},"id":"17955","type":"LinearAxis"},{"attributes":{},"id":"17968","type":"UndoTool"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"18013","type":"Circle"},{"attributes":{},"id":"17966","type":"WheelZoomTool"},{"attributes":{"coordinates":null,"formatter":{"id":"17997"},"group":null,"major_label_policy":{"id":"17998"},"ticker":{"id":"17960"}},"id":"17959","type":"LinearAxis"},{"attributes":{},"id":"17953","type":"LinearScale"},{"attributes":{},"id":"18000","type":"BasicTickFormatter"},{"attributes":{"children":[{"id":"18036"},{"id":"18034"}]},"id":"18037","type":"Column"},{"attributes":{},"id":"17949","type":"DataRange1d"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17992"}]},"id":"18009","type":"LegendItem"},{"attributes":{"below":[{"id":"17955"}],"center":[{"id":"17958"},{"id":"17962"},{"id":"18008"}],"height":500,"left":[{"id":"17959"}],"output_backend":"webgl","renderers":[{"id":"17986"},{"id":"17992"},{"id":"18014"}],"title":{"id":"18016"},"toolbar":{"id":"17973"},"toolbar_location":null,"width":500,"x_range":{"id":"17947"},"x_scale":{"id":"17951"},"y_range":{"id":"17949"},"y_scale":{"id":"17953"}},"id":"17946","subtype":"Figure","type":"Plot"}],"root_ids":["18037"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"5b410095-192d-449d-bed1-2338e7913fbc","root_ids":["18037"],"roots":{"18037":"ed5a6ca3-5838-43d6-a772-6c0497b2346d"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();