(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("5193de7c-6abf-434a-befe-08231a257849");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '5193de7c-6abf-434a-befe-08231a257849' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.1.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"c259f0b3-aebb-4472-9233-4eedb28c22e2":{"defs":[],"roots":{"references":[{"attributes":{},"id":"22188","type":"UnionRenderers"},{"attributes":{"coordinates":null,"data_source":{"id":"22166"},"glyph":{"id":"22167"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"22169"},"nonselection_glyph":{"id":"22168"},"view":{"id":"22171"}},"id":"22170","type":"GlyphRenderer"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[100]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[100]}},"selected":{"id":"22189"},"selection_policy":{"id":"22188"}},"id":"22172","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"formatter":{"id":"22181"},"group":null,"major_label_policy":{"id":"22182"},"ticker":{"id":"22144"}},"id":"22143","type":"LinearAxis"},{"attributes":{"fill_alpha":0.1,"fill_color":"#ff0000","hatch_alpha":0.1,"line_alpha":0.1,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22168","type":"Patch"},{"attributes":{"axis":{"id":"22139"},"coordinates":null,"group":null,"ticker":null},"id":"22142","type":"Grid"},{"attributes":{"coordinates":null,"data_source":{"id":"22172"},"glyph":{"id":"22173"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"22175"},"nonselection_glyph":{"id":"22174"},"view":{"id":"22177"}},"id":"22176","type":"GlyphRenderer"},{"attributes":{},"id":"22187","type":"Selection"},{"attributes":{},"id":"22135","type":"LinearScale"},{"attributes":{},"id":"22148","type":"PanTool"},{"attributes":{},"id":"22133","type":"DataRange1d"},{"attributes":{},"id":"22137","type":"LinearScale"},{"attributes":{},"id":"22140","type":"BasicTicker"},{"attributes":{"tools":[{"id":"22147"},{"id":"22148"},{"id":"22149"},{"id":"22150"},{"id":"22151"},{"id":"22152"},{"id":"22153"},{"id":"22154"}]},"id":"22157","type":"Toolbar"},{"attributes":{"fill_alpha":0.2,"fill_color":"#ff0000","hatch_alpha":0.2,"line_alpha":0.2,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22169","type":"Patch"},{"attributes":{},"id":"22185","type":"AllLabels"},{"attributes":{"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22173","type":"Line"},{"attributes":{},"id":"22184","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"22155"}},"id":"22149","type":"BoxZoomTool"},{"attributes":{},"id":"22181","type":"BasicTickFormatter"},{"attributes":{},"id":"22152","type":"UndoTool"},{"attributes":{"axis":{"id":"22143"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"22146","type":"Grid"},{"attributes":{"source":{"id":"22166"}},"id":"22171","type":"CDSView"},{"attributes":{"source":{"id":"22172"}},"id":"22177","type":"CDSView"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"22155","type":"BoxAnnotation"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"22156","type":"PolyAnnotation"},{"attributes":{"callback":null},"id":"22154","type":"HoverTool"},{"attributes":{},"id":"22131","type":"DataRange1d"},{"attributes":{},"id":"22186","type":"UnionRenderers"},{"attributes":{},"id":"22144","type":"BasicTicker"},{"attributes":{},"id":"22189","type":"Selection"},{"attributes":{"coordinates":null,"group":null},"id":"22178","type":"Title"},{"attributes":{},"id":"22147","type":"ResetTool"},{"attributes":{"overlay":{"id":"22156"}},"id":"22151","type":"LassoSelectTool"},{"attributes":{},"id":"22150","type":"WheelZoomTool"},{"attributes":{"coordinates":null,"formatter":{"id":"22184"},"group":null,"major_label_policy":{"id":"22185"},"ticker":{"id":"22140"}},"id":"22139","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[400]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[400]}},"selected":{"id":"22187"},"selection_policy":{"id":"22186"}},"id":"22166","type":"ColumnDataSource"},{"attributes":{},"id":"22153","type":"SaveTool"},{"attributes":{},"id":"22182","type":"AllLabels"},{"attributes":{"below":[{"id":"22139"}],"center":[{"id":"22142"},{"id":"22146"}],"height":500,"left":[{"id":"22143"}],"output_backend":"webgl","renderers":[{"id":"22170"},{"id":"22176"}],"title":{"id":"22178"},"toolbar":{"id":"22157"},"toolbar_location":"above","width":500,"x_range":{"id":"22131"},"x_scale":{"id":"22135"},"y_range":{"id":"22133"},"y_scale":{"id":"22137"}},"id":"22130","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.2,"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22175","type":"Line"},{"attributes":{"fill_alpha":0.5,"fill_color":"#ff0000","line_alpha":0,"line_color":"#ff0000","x":{"field":"x"},"y":{"field":"y"}},"id":"22167","type":"Patch"},{"attributes":{"line_alpha":0.1,"line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"22174","type":"Line"}],"root_ids":["22130"]},"title":"Bokeh Application","version":"2.4.1"}}';
                  const render_items = [{"docid":"c259f0b3-aebb-4472-9233-4eedb28c22e2","root_ids":["22130"],"roots":{"22130":"5193de7c-6abf-434a-befe-08231a257849"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();